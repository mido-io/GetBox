...
function isLoginPage(html) {
  ...
  return /(?:You must log in|login_form|<title[^>]*>Log In|checkpoint\.php|login\?next=|\/login\/|"is_logged_in"\s*:\s*false)/i.test(sample);
}

function extractVideoId(url) {
  try {
    const u = new URL(url.toString());
    const m = u.pathname.match(/\/(?:reel|video(?:s)?)\/(\d{8,})/i);
    if (m) return m[1];
...
// Try Facebook's video embed plugin endpoint — works for many public videos without login
async function tryFbPluginEmbed(videoId) {
  const src = `https://www.facebook.com/video/embed/?video_id=${videoId}`;
  const html = await safeFetch(src, {
    ua: DESKTOP_UA,
    timeoutMs: 8000,
    headers: { Referer: "https://www.facebook.com/" },
  });
  if (!html || isLoginPage(html)) return { hd: null, sd: null, images: [] };
...
export async function resolveFacebook(url) {
  ...
  for (const { u, ua } of urlsToTry) {
    ...
    if (!html || isLoginPage(html)) continue;
  ...
  // Fallback: try the FB video embed plugin — bypasses login for public reel/video URLs
  if (!items.length) {
    const videoId = extractVideoId(url);
    if (videoId) {
      const { hd, sd } = await tryFbPluginEmbed(videoId);
      if (hd) {
        items.push({
          type: "video",
          url: hd,
